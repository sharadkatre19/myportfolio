...
// Define the type for skill names
type SkillName = 
  | 'gcp' | 'html' | 'photoshop' | 'docker' | 'illustrator' | 'adobe xd' 
  | 'after effects' | 'css' | 'angular' | 'javascript' | 'next js' | 'nuxt js'
  | 'node js' | 'react' | 'svelte' | 'typescript' | 'vue' | 'bootstrap' | 'bulma'
  | 'capacitorjs' | 'coffeescript' | 'memsql' | 'mongodb' | 'mysql' | 'postgresql'
  | 'tailwind' | 'vitejs' | 'vuetifyjs' | 'c' | 'c++' | 'c#' | 'dart' | 'go'
  | 'java' | 'kotlin' | 'julia' | 'matlab' | 'php' | 'prisma' | 'python' | 'ruby'
  | 'swift' | 'adobe audition' | 'aws' | 'deno' | 'django' | 'firebase' | 'gimp'
  | 'git' | 'graphql' | 'lightroom' | 'materialui' | 'nginx' | 'numpy' | 'opencv'
  | 'premiere pro' | 'pytorch' | 'selenium' | 'strapi' | 'tensorflow' | 'webix'
  | 'wordpress' | 'azure' | 'blender' | 'fastify' | 'figma' | 'flutter' | 'haxe'
  | 'ionic' | 'markdown' | 'microsoft office' | 'picsart' | 'sketch' | 'unity'
  | 'wolframalpha' | 'canva' | 'pandas' | 'sklearn' | '.net' | '.net core'
  | 'kubernetes' | 'linux' | 'sqlalchemy' | 'fastapi';

export const skillsImage = (skill: string): string | undefined => {
  const skillID = skill.toLowerCase() as SkillName;
